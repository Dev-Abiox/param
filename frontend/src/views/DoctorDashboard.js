...
const DoctorDashboard = ({ user }) => {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-96 text-slate-400 dark:text-slate-500" data-testid="doctor-dashboard-loading">
  ...
  return (
    <div className="space-y-6" data-testid="doctor-dashboard">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Welcome, Dr. {user?.name || "Doctor"}</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400">Your patient overview and recent activity</p>
        </div>
        <div className="flex items-center px-4 py-2 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded-lg border border-blue-200 dark:border-blue-800">
      ...
      {alerts.length > 0 && (
        <div className="space-y-2">
          {alerts.map((alert, i) => (
            <div key={`${alert.screening_id}-${i}`} className="flex items-center gap-3 px-4 py-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900 p-5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between" data-testid="stat-total-patients">
          <div>
            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Total Patients</p>
            <h3 className="text-3xl font-extrabold text-slate-800 dark:text-slate-100 mt-1">{stats?.totalPatients || stats?.totalCases || 0}</h3>
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between" data-testid="stat-screenings-today">
          <div>
            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Screenings Today</p>
            <h3 className="text-3xl font-extrabold text-slate-800 dark:text-slate-100 mt-1">{stats?.screeningsToday || stats?.dailyTests || 0}</h3>
            <p className="text-xs text-green-600 font-medium mt-1 flex items-center">
              <TrendingUp className="w-3 h-3 mr-1" /> Active
            </p>
          </div>
          <div className="p-3 rounded-full bg-green-50 dark:bg-green-900/30 text-green-600 border border-green-100 dark:border-green-800">
            <Activity className="w-6 h-6" />
          </div>
        </div>

        <div className="bg-white dark:bg-slate-900 p-5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between" data-testid="stat-deficient">
          <div>
            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Deficient Cases</p>
            <h3 className="text-3xl font-extrabold text-slate-800 dark:text-slate-100 mt-1">{stats?.deficientCases || stats?.distribution?.[2]?.value || 0}</h3>
        ...
        <div className="bg-white dark:bg-slate-900 p-5 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between" data-testid="stat-normal">
          <div>
            <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">Normal Results</p>
            <h3 className="text-3xl font-extrabold text-slate-800 dark:text-slate-100 mt-1">{stats?.normalCases || stats?.distribution?.[0]?.value || 0}</h3>
            <p className="text-xs text-green-600 font-medium mt-1 flex items-center">
      ...
      {/* Recent Screenings */}
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-sm overflow-hidden" data-testid="recent-patients">
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
          <div className="flex items-center">
            <FileText className="w-5 h-5 text-blue-600 mr-2" />
            <h4 className="font-bold text-slate-700 dark:text-slate-300">Recent Screenings</h4>
          </div>
          <span className="text-xs text-slate-400 dark:text-slate-500">Last 5 entries</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700 text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800">
              <tr>
                <th className="px-6 py-3 text-left font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-xs">Patient ID</th>
                <th className="px-6 py-3 text-left font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-xs">Date</th>
                <th className="px-6 py-3 text-left font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider text-xs">Patient</th>
            ...
            <tbody className="bg-white dark:bg-slate-900 divide-y divide-slate-100 dark:divide-slate-700">
              {(stats?.recentCases || stats?.recentPatients || []).slice(0, 5).map((item, index) => (
                <tr key={item.id || index} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  <td className="px-6 py-3 font-mono text-xs text-slate-600 dark:text-slate-300">{item.id || item.patientId}</td>
                  <td className="px-6 py-3 text-slate-600 dark:text-slate-300">{item.date}</td>
                  <td className="px-6 py-3 font-medium text-slate-800 dark:text-slate-100">{item.patientRef || item.name || "N/A"}</td>
